...
- წიგნის ბოლოს არის დიდი index და თითო თავის შემდეგ bibliography - ის chunk-ები ამოვაგდეთ
- PDF-იდან რჩებოდა letter-spaced სათაურები (`N EURAL N ETWORKS`) და `[M]achines` - ესეც გავწმინდეთ
- საბოლოოდ **712 chunk** (200-300 სიტყვა)
...
- ძალიან მოკლე document-ები (<40 სიტყვა) ამოვაგდეთ - პატარა chunk retrieval-ში სუსტია
...
<img src="data_split.png" width="800" height="300">

J&M demo corpus: **712 chunk** (მთელი წიგნი), თითო 200-300 სიტყვა.
...
<img src="data_lengths.png" width="600" height="300">
<img src="query_starts.png" width="600" height="300">
<img src="questions_per_doc.png" width="600" height="300">
...
- წვრთნა არ სჭირდება, ამიტომ კარგი  შესადარებელია ჩვენი მოდელისთვის

BM25-ს ზუსტად იგივე index-ზე და იგივე test query-ებზე ვუშვებთ, რაზეც ჩვენს მოდელს
...
~22M პარამეტრი). ძალიან სწრაფად ოუვერფიტში წავიდა - train loss იკლდებდა, val loss კი არ მცირდებოდა.
რადგან მონაცემები scratch მოდელისთვის შედარებით ცოტაა და overfitting პრობლემა იყო, **შევამცირეთ
მოდელი** (256/4/2) და dropout 0.3-მდე ავწიეთ რეგულარიზაციის მხრივ. პატარა მოდელი უკეთ განზოგადდა, ამიტომ ეს დავტოვეთ
ძირითად მოდელად.

პირველი ექსპერიმენტის პარამეტრები და შედეგები:
...
| dropout | 0.1 |
| პარამეტრების რაოდენობა | 22,066,816 |
...
train loss 1.95-დან 0.63-მდე ჩამოვიდა 4 epoch-ში, val loss კი 2.88-ზე დარჩა —
overfitting-ის კლასიკური სურათი. ამის შემდეგ გადავწყვიტეთ მოდელი შეგვემცირებინა.
...
გრაფზე ჩანს: train loss (ლურჯი) სწრაფად ეცემა 2.76-დან 0.73-მდე, val loss (ნარინჯისფერი) 
კი თითქმის ჰორიზონტალურია ~2.7-ზე. აქაც **overfitting** გვაქ - მოდელი 
train მონაცემებს კარგად სწავლობს, მაგრამ ახალ მონაცემებზე განზოგადება უჭირს.

რატომ ხდება ეს ჩვენს შემთხვევაში:
1. **scratch მოდელი** — BERT-ისგან განსხვავებით, ჩვენი მოდელი ნულიდან იწყებს. მას 
   არ აქვს წინასწარი ცოდნა ენის შესახებ და მთლიანად 68k წყვილზეა დამოკიდებული.

2. **მონაცემების სიმცირე** — 68k წყვილი scratch Transformer-ისთვის ცოტაა. 
   ნამდბილი მოდელები გაწვრთნილია მილიარდობით სიტყვაზე — ჩვენი მოდელი კი მხოლოდ 68k-ზე.

3. **SQuAD-ის სტრუქტურა** — ერთი document საშუალოდ 5 კითხვას ემსახურება 
   (dup_doc_rate 0.79). მოდელმა train-ში ისწავლა კონკრეტული (query, document) 
   წყვილები, მაგრამ val-ში იგივე document ახალ კითხვასთან წყვილშია — 
   ეს განზოგადებას ართულებს.

4. **in-batch false negatives** — InfoNCE batch-ში სხვა document-ებს negative-ად 
   იყენებს. SQuAD-ში ერთი document რამდენიმე query-ს positive-ია, ამიტომ 
   ზოგი negative სინამდვილეში positive-ია.

საუკეთესო val loss (2.7040) მე-10 epoch-ზე იყო, ეს checkpoint 
(`checkpoints/squad_v2/best_model.pt`) დავტოვეთ საბოლოო ევალუაციისთვის.
...
აქ სულ სხვა სურათია: train loss ეცემა, მაგრამ val loss **იზრდება** (3.78 -> 5.07). ანუ მოდელი
train-ს იზეპირებს, მაგრამ განზოგადება საერთოდ უარესდება. მიზეზი ისაა, რომ Wiki query (1-2 სიტყვიანი
...
თავში მოხვდეს. Recall@1 აჩვენებს ზუსტად მოხვდა თუ არა, Recall@5 - top-k-ში მოხვედრას,
...
BM25 ჩვენს მოდელს მკაფიოდ **ჯობს** ყველა მეტრიკაში, რაც მოსალოდნელი იყო:

1. **scratch მოდელი + ცოტა მონაცემები.** BM25-ს წვრთნა არ სჭირდება, ჩვენმა მოდელმა კი ნულიდან უნდა
   ისწავლოს ენა ~68k წყვილზე - ეს ცოტაა იმისთვის, რომ Transformer-მა კარგი რეპრეზენტაცია ისწავლოს.
...
3. **SQuAD-ში პასუხი ხშირად query-ის სიტყვებს შეიცავს** - ეს keyword overlap-ს ეხმარება, ანუ
   BM25-ისთვის სასარგებლო dataset-ია.
...
demo (`demo.py`) Jurafsky & Martin წიგნის corpus-ზე (712 chunk, მთელი წიგნი) მუშაობს. მომხმარებელი
...
რამდენიმე მაგალითი (default `squad_v2` მოდელი, top-3):

**query: "what are neural networks"** - კარგი შემთხვევა, top-5 ყველა neural networks-ზეა, #1 ზუსტად სწორია:
1. neural networks თავის შესავალი (mcculloch-pitts neuron) - სწორი
2. recurrent neural networks
3. feedforward neural networks
4. neural units summary
5. neural net initialization / regularization
...
- დასკვნა : contrastive retrieval-ში მონაცემების ხარისხი და რაოდენობა ისეთივე
  მნიშვნელოვანია, როგორც არქიტექტურა
---
